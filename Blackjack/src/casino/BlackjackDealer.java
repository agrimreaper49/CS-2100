...
public class BlackjackDealer {
	...
	int playerChips; // set when BlackjackPlayer.setDealer() called to connect player and dealer
	...
	private void hitDealer() {
		this.dealerHand.add(decks.dealTopCard());
	}
	...
	protected void playHand(int handNum) {
		/* Clear the player hands */
		this.player.cards = new Card[this.player.cards.length]; // create new empty array
		...
		int win = 0;
		if (d == 21 && this.dealerHand.size() == 2)
			; // do nothing, keep player bet
		else if (p > 21)
			; // player busts
		else if (d > 21)
			win = bet * 2; // dealer busts, payout is 1.5x
		else if (p == 21 && this.playerHand.size() == 2)
			win = bet + ((int) (bet * 1.5)); // blackjack! payout is 2x
		else if (p > d)
			win = bet * 2; // player wins, payout is 1.5x
		else if (p == d)
			win = bet; // tie, player gets the bet back
...
//		System.out.println("Playr Hand2: " + Arrays.asList(this.player.cards));
